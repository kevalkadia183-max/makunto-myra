...
voxRouter.post("/vox/greeting", async (req, res) => {
  ...
  const greetPersonality = (req.body as { personality?: string } | undefined)?.personality;
  ...
  if (!greetDemo && isConfigured() && isVerifiedUser(clientId)) {
    try {
      ...
      if (report.length > 0) {
        ...
        dataContext += "\n\nNOTE: No accounts connected yet. Greet them like a friendly human, mention something interesting from the trending list below as a conversation starter, and casually invite them to connect their account in Setup so you can talk about their own channel.";
        try {
